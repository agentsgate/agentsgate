...
def draw_diagram(out_path, t):
    ...
    for num, title, l1, l2, tag in steps:
        ...
        if l2:
            # Monospace suits the score/category examples, but Menlo has no CJK
            # glyphs — a Japanese prefix on that line renders as tofu. Choose by
            # what the string actually contains, so rewording it cannot
            # reintroduce the problem.
            mono_ok = num == '①' and l2.isascii()
            text(rail + 58, y + 48, l2, f_mono if mono_ok else f_body,
...
EN = dict(
    ...
    through='every MCP tool call passes through here',
    s1='Risk rules', s1a='What kind of operation is this, and how risky?',
    s1b='Exp) DROP TABLE → 1.00 "destructive"    write a file → 0.65 "write_update"',
    ...
    s3b='Exp) block writes under /etc  ·  allow rm -rf node_modules',
...
JA = dict(
    ...
    through='すべてのMCPツール呼び出しがここを通る',
    s1='リスクルール', s1a='どんな種類の操作か、どれくらい危険か',
    s1b='例）DROP TABLE → 1.00 "destructive"    file write → 0.65 "write_update"',
    s2='保護レベル', s2a='そのカテゴリをどう扱うか',
    s2b='minimal   /   balanced（既定）   /   strict',
    s3='ポリシールール', s3a='個別の例外 — レベルを上書きし、厳しくも緩くもできる',
    s3b='例）/etc 配下への書き込みを拒否  ·  rm -rf node_modules を許可',
